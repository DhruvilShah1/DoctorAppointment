...
io.on("connection", (socket) => {
  ...
  socket.on("patient:join", ({ doctorId, date, slot }) => {
    const roomId = `${doctorId}_${date}_${slot}`;

    socket.join(roomId);

    console.log("✅ Patient joined:", roomId);

   console.log(socket.rooms);
  });

  socket.on("doctor:join", ({ doctorId, date, slot }) => {
    const roomId = `${doctorId}_${date}_${slot}`;

    console.log(" Queue start emit to:", roomId);

    io.to(roomId).emit("queue:started", {
      date , slot , 
      message: "Doctor has started the queue",
    });
