...
const ScheduleDoctor = () => {
  ...
  const authFetch = async (url, options = {}) => {
        ...
        const res = await fetch(`${VITE_BACKEND_URL}/api/refresh-token`, {
      method: "POST",
      credentials: "include",
    });
  ...
  const loadTodayData = async () => {
    try {
      ...
      const [totalRes, slotRes] = await Promise.all([
        fetch(`${VITE_BACKEND_URL}/api/total/patient/day`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        fetch(`${VITE_BACKEND_URL}/api/get/slots`, {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const loadPatients = async (slot) => {
    try {
      const res = await authFetch(`${VITE_BACKEND_URL}/api/take/patient`,
        {
          method: "POST",
          body: JSON.stringify({
            date: today,
  ...
  if (patients.length === 0) {
    ...
    try {
      const res = await authFetch(
        `${VITE_BACKEND_URL}/api/start/queue`,
        {
          method: "POST",
          body: JSON.stringify({
            date: today,
  ...
  const updatePatientStatus = async (status) => {
    ...
    try {
      const res = await authFetch(
        `${VITE_BACKEND_URL}/api/update/patient/status`,
        {
          method: "PATCH",
          body: JSON.stringify({
            date: today,
  ...
  const finishSlot = async () => {
    try {
      const res = await authFetch(
        `${VITE_BACKEND_URL}/api/finsh/slot`,
        {
          method: "POST",
          body: JSON.stringify({
            date: today,
